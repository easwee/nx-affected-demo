...
const getTouchedProjectsFromTsConfig = (touchedFiles, _a, _b, _c, graph) => {
    ...
    const sortedNodes = (0, project_graph_1.getSortedProjectNodes)(graph.nodes);
...
function getProjectsAffectedByPaths(change, sortedNodes) {
    ...
    paths.forEach((path) => {
        sortedNodes.forEach((project) => {
            if (path &&
                project.data.root &&
                (0, path_1.join)(app_root_1.appRootPath, path).startsWith((0, path_1.join)(app_root_1.appRootPath, project.data.root))) {
